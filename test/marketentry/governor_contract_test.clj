  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Spice Isle Construction Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Spice Isle Construction Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest director-conviction-disqualifying-is-held-and-unoverridable
  (testing "director/officer conviction still within the s.17(1)(f) two-year lookback -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
